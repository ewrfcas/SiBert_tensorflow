...
class bert_model(object):
    def __init__(self,
        ...
        with tf.variable_scope("bert_model", reuse=not train,
            ...
            with tf.device("/gpu:0"):
                ...
                # finetune cmrc
                with tf.variable_scope('finetune_cmrc'):
                    # [bs, len]
                    self.logits = tf.squeeze(self.conv1d(self.pooled_output, 'start_dense', 1), -1)
                    self.logits += (-10000. * tf.cast((1 - answer_mask), self.logits.dtype))

                    if train and positions is not None:
                        # [bs, len]
